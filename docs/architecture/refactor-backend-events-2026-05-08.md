...
## Сравнение с kumho-tires (как там решены те же моменты)

Проверил эталонный проект — большинство «открытых вопросов» **унаследованы оттуда** или специфичны для italy. Сводка:

| #   | Вопрос                               | kumho                                                                 | Вывод                                                          |
| --- | ------------------------------------ | --------------------------------------------------------------------- | -------------------------------------------------------------- |
| 1   | Mail DSN не сконфигурирован          | ✅ `.env.example` с MAILER_DSN/MAIL_TO/MAIL_FROM                      | Скопировал блок в italy `.env.example`                         |
| 2   | routeParams.length > 1               | ❌ Тоже падает на silent break, без 404                               | Унаследовано, чинить общим решением                            |
| 3   | route_map vs entity slug             | ❌ Тот же приоритет route_map                                         | Унаследовано                                                   |
| 4   | prod_base_url хардкодом              | ➖ kumho не делает абсолютные URL для og в коллекциях                 | Italy-специфика (рестораны хотят shareable URL на прод-домене) |
| 5   | 404 без entity-fallback              | ❌ Та же логика (Кейс A → 404)                                        | Унаследовано                                                   |
| 6   | Listener'ы не подписаны              | ❌ Тоже `EventDispatcher` без подписчиков                             | Архитектура готова в обоих, потребителей нет                   |
| 7   | `buildEntityBreadcrumb` в PageAction | ❌ Тоже inline, без отдельного сервиса                                | Унаследовано                                                   |
| 8   | composer install --no-dev            | ➖ Тот же паттерн                                                     | Норма (процессное, не ошибка)                                  |
| 9   | `ContainerInterface` в PageAction    | ➖ kumho НЕ использует — у них generic SEO без per-collection builder | Italy-специфика (моя добавка для RestaurantSeoBuilder)         |

Итого только пункт 1 действительно «решён в kumho и не перенесён» — поправлено. Пункты 2/3/5/6/7 — унаследованные ограничения архитектуры; их стоит починить, но это работа для следующего рефакторинга в обоих проектах сразу. Пункты 4/9 — italy-специфика, в kumho нет.

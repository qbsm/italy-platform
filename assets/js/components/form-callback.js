document.addEventListener('DOMContentLoaded', function () {
  ...
  function getLocalizedText(category, key, defaultValue, langOverride = null) {
    ...
    try {
      if (
        window.globalData &&
        window.globalData['form-callback'] &&
  ...
  if (typeof window.url !== 'function') {
    console.error(
      'Критическая ошибка: функция window.url() не определена. Проверьте, что window.appConfig.baseUrl установлен.'
    );
  ...
  fetch(window.url('data/json/global.json'))
    .then((response) => {
      if (!response.ok) {
        console.error(
          'Ошибка загрузки global.json: ' + response.status + ' ' + response.statusText + ' URL: ' + response.url
        );
  ...
  function loadCountryCodes(callback) {
    ...
    fetch(window.url('assets/js/data/country-codes.json'))
      .then((response) => {
        if (!response.ok) {
          console.error(
            'Ошибка загрузки country-codes.json: ' +
              response.status +
              ' ' +
              response.statusText +
              ' URL: ' +
      ...
      .catch((error) => {
        console.error(
          'Критическая ошибка: не удалось загрузить country-codes.json. Маска телефона может не работать корректно.',
          error
        );
  ...
  function initForm(form, countryCodes) {
    ...
    const formFieldsForInputReset = form.querySelectorAll(
      'input[name="name"], input[name="phone"], input[name="square"], input[name="email"]'
    );
  ...
  function initPhoneMask(form, countryCodes) {
          ...
          if (phoneDigits.length > 0) {
            ...
            if (phoneDigits.length <= 2) {
              ...
              if (phoneDigits.length > currentPos) {
                formatted += '-' + phoneDigits.substring(currentPos, Math.min(currentPos + 3, phoneDigits.length));
                currentPos += 3;
                if (phoneDigits.length > currentPos) {
      ...
      if (this.value && this.value.indexOf('@') !== -1) {
        ...
        if (cleanDigits.length >= 10) {
          if (cleanDigits.charAt(0) === '9' && cleanDigits.length === 10) {
            ...
            this.setAttribute('data-detected-format', 'russia');
            this.setAttribute('data-country-code', '+7');
            return;
          } else {
            ...
            const code = countryInfo
              ? countryInfo.code
              : '+' + cleanDigits.substring(0, Math.min(cleanDigits.length, 3));
          ...
          this.value = cleanPhoneNumber;
        }
      }
      if (!this.value.trim()) {
        const htmlLang = document.documentElement.lang ? document.documentElement.lang.toLowerCase() : 'ru';
        let defaultCode = '+7';
        let defaultFormat = 'russia';
        if (htmlLang === 'en') {
          defaultCode = '+1';
          defaultFormat = 'north-america';
        } else if (htmlLang === 'de') {
          defaultCode = '+49';
          defaultFormat = 'standard';
        }
        this.value = defaultCode;
        this.setAttribute('data-country-code', defaultCode);
        this.setAttribute('data-detected-format', defaultFormat);
      } else {
    ...
    phoneInput.addEventListener('blur', function () {
      ...
      if (digitsOnly === countryCodeDigits || digitsOnly.length <= (countryCodeDigits ? countryCodeDigits.length : 1)) {
        if (!this.value.match(/[1-9]/) || this.value.replace(/\D/g, '').length < 2) {
          ...
          this.removeAttribute('data-detected-format');
    ...
    phoneInput.addEventListener('input', function () {
      ...
      if (digits.length === 1 && originalValue.charAt(0) !== '+') {
        this.value = '+' + digits;
        this.removeAttribute('data-country-code');
        this.removeAttribute('data-detected-format');
        return;
      ...
      if (countryInfo) {
        ...
        let tempCode = '+';
        if (digits.length > 0) tempCode += digits.substring(0, 1);
        if (digits.length > 1) tempCode += digits.substring(1, 2);
        formattedNumber = formatPhoneNumber(digits, 'standard', tempCode.length > 1 ? tempCode : null);
        this.removeAttribute('data-country-code');
        this.setAttribute('data-detected-format', 'standard');
      }
    ...
    phoneInput.addEventListener('change', function () {
      if (this.value && this.value.trim() !== '') {
        ...
        if (digits.length === 1 && this.value.charAt(0) !== '+') {
          this.value = '+' + digits;
          return;
        }
        const countryInfo = detectCountryCode(digits);
    ...
    phoneInput.addEventListener('animationstart', (e) => {
      if (e.animationName.includes('autofill'))
        setTimeout(() => phoneInput.dispatchEvent(new Event('change', { bubbles: true })), 50);
    });
  }
  ...
  function validateForm(form) {
    ...
    if (nameField && nameVisible) {
      if (!nameField.value.trim()) {
        ...
        markFieldAsError(nameField, msg);
        if (!firstErrorMessage) firstErrorMessage = msg;
        isValid = false;
    ...
    if (squareField && squareVisible && !squareField.value.trim()) {
      ...
      markFieldAsError(squareField, msg);
      if (!firstErrorMessage) firstErrorMessage = msg;
      isValid = false;
    }
    ...
    if (emailField) {
      ...
      if (emailValue && !emailRegex.test(emailValue)) {
        ...
        markFieldAsError(emailField, msg);
        if (!firstErrorMessage) firstErrorMessage = msg;
        isValid = false;
      }
    }
  ...
  /** @param {HTMLElement} fieldElement @param {string} [message] — не используется, оставлен для единообразия вызова */
  ...
  function sendForm(form) {
    ...
    const hasUtmSession = (function () {
      try {
        return sessionStorage.getItem('utm_session') === '1';
      } catch {
        return false;
      }
    })();
    ...
    if (hasUtmSession || hasUtmInUrl) {
      utmParams.forEach((param) => {
        let value = null;
        try {
          value = sessionStorage.getItem(param) || null;
        } catch {
          /* ignore */
        }
        ...
        if (!value) {
          ...
          value =
            window.utmHelper && typeof window.utmHelper.getCookie === 'function'
    ...
    const utmKeysForGet = ['utm_source', 'utm_medium', 'utm_campaign', 'utm_term', 'utm_content'];
    ...
    xhr.onload = function () {
      ...
      try {
        ...
        try {
          ...
          serverResponse = {
            success: false,
            message: getLocalizedText('error', 'response_error', 'Произошла ошибка при обработке ответа сервера'),
          };
        }
        if (serverResponse.success) {
          ...
          if (serverResponse.errors) {
            for (const fieldName in serverResponse.errors) {
              ...
              if (field) {
                ...
                if (!displayedError) {
                  showFormError(form, serverResponse.errors[fieldName]);
                  displayedError = true;
                }
              }
            }
          ...
          if (!displayedError && serverResponse.message) {
            ...
            const generalErrorKey = serverResponse.message
              ? null
              : getLocalizedText('error', 'form_errors', null)
                ? 'form_errors'
                : 'server_error';
            if (generalErrorKey) {
  ...
  function checkSubmissionStatus(form, submitButton, originalButtonContent, serverResponse) {
    ...
    const requestId =
      serverResponse && serverResponse.request_id ? serverResponse.request_id : Math.floor(Date.now() / 1000); // Fallback, если не получен из первого ответа
    ...
    /* eslint-disable no-unreachable -- код для будущей поддержки api/status */
    let attempts = 0;
    const maxAttempts = 3;
        ...
        try {
          ...
          if (response.completed) {
            ...
            if (response.success) {
              ...
              showFormError(
                form,
                response.message || getLocalizedText('error', 'server_error', 'Произошла ошибка при отправке.')
              );
            }
            ...
            form.dispatchEvent(
              new CustomEvent('formSubmissionSuccess', {
                ...
                detail: {
                  form: form,
                  serverResponse: { success: true, message: 'Заявка отправлена, но статус не подтвержден' },
                },
              })
            );

...
final class DataLoaderService
{
     ...
     * @return array<string,mixed>|null
     */
    public function loadEntity(string $jsonBaseDir, string $langCode, string $slug, string $baseUrl, array $collectionConfig, bool $includeHidden = false): ?array
    {
        ...
        // visible:false — скрытая сущность: не попадает в списки/афишу и sitemap.
        // Детальная страница остаётся ДОСТУПНОЙ (200) при $includeHidden=true — чтобы
        // уже проиндексированные/расшаренные URL не отдавали 404; PageAction помечает
        // такую страницу noindex, и поисковик снимает её из индекса без ошибок обхода.
